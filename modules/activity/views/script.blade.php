<script>
    ...
    function titleFormatter(value, row, index) {
        return '<a href="/admin/activities/' + row.id + '" target="_blank">' + row.title + '</a>' +
    ...
    window.actionEvents = {
        ...
        'click .top': function (e, value, row, index) {
            $.ajax({
                url: '/admin/activities/' + row.id + '/top',
                type: 'post',
        ...
        'click .tag': function (e, value, row, index) {
            $.ajax({
                url: '/admin/activities/' + row.id + '/tag',
                type: 'post',
        ...
        'click .count': function (e, value, row, index) {
            ...
            $('#modal_title').text('活动统计');
            $('#window_msg').hide();

            var url = '{{$base_url}}/data/' + row.id;
            $.ajax({
                url: url,
                type: "get",
                data: {'_token': '{{ csrf_token() }}'},
