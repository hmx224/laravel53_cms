<script>
    ...
    function titleFormatter(value, row, index) {
        return '<a href="/article/detail-' + row.id + '.html" target="_blank">' + row.title + '</a>' +
    ...
    /*查看评论*/
    function comment_video(id) {
        $('.common').prop('id', 'modal_comment');
        $('#modal_title').text('查看评论');
        $('#window_msg').hide();

        var url = '{{$base_url}}/comments/' + id;
        $.ajax({
            url: url,
            type: "get",
            data: {'_token': '{{ csrf_token() }}'},
    ...
    /*推送*/
    function push_video(id) {
        var id_video = id + '_push';
        $('#push_id').val(id);
        $('#push_title').val($('.' + id_video).val());
    }
