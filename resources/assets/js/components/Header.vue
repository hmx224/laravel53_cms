        ...
        <header class="header">
            ...
            <div class="navbar">
                <div class="navbar-left-container">
                    <a href="/mall">
                        <img class="navbar-brand-logo" src="/images/mall/logo1.png"></a>
                </div>
                <div class="navbar-right-container" style="display: flex;">
                    <div class="navbar-menu-container">
                        ...
                        <div class="navbar-cart-container">
                            ...
                            <a class="navbar-link navbar-cart-link" href="/mall/cart">
                        ...
                        <div class="confirm-tips">
                            ...
                            <ul>
                                <li class="regi_form_input">
                                    ...
                                    <input type="text" tabindex="1" name="loginname" v-model="memberName"
...
<script>
    export default {
        ...
        methods: {
            init() {
                let reg = new RegExp('(^| )app_token=([^;]*)(;|$)');
                let arr = document.cookie.match(reg);
                if (arr) {
                    let token = unescape(arr[2]);
                    if (token) {
                        axios.get('/api/members/info?token=' + token).then((response) => {
                            if (response.data.status_code == 200) {
                                let member = response.data.data;
                                this.nickname = member.nick_name;
                            }
                            else {
                                this.logout();
                            }
            ...
            login() {
                if (!this.memberName || !this.password) {
                    this.errorTip = true;
                    return;
                }
                axios.get('/api/members/login?member_name=' + this.memberName + '&password=' + this.password).then((response) => {
                    if (response.data.status_code == 200) {
                        let member = response.data.data;
                        this.errorTip = false;
                        ...
                        document.cookie = 'app_token=' + member.token + '; ' + expires + ';path=/';
                    }
                    else {
                        this.errorTip = true;
                    }
            ...
            logout() {
                this.nickname = '';
                document.cookie = 'app_token=' + '' + ';path=/';
            }
        }
    }
</script>
